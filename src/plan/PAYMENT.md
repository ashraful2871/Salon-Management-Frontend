...
### Step P3.4 — Show the customer what's happening

In the booking modal, above the Confirm button:
...
│  Pay at salon                          ৳75   │
│                                              │
│  ℹ Your ৳30 comes off the bill when you      │
│    arrive. Cancel before 10:30 AM and it     │
│    goes straight back to your wallet.        │
│                                              │
│  Wallet: ৳470 available          [Confirm]   │
└──────────────────────────────────────────────┘
```
...
3. Book, complete     → balance ৳470, held ৳0, DEPOSIT_APPLIED row
...
### Step P4.4 — "Collect at salon" screen

The owner's counter screen. Open appointment → **Complete & Collect**:
...
│  [ Cash ]  [ bKash ]  [ Card ]         │
│                                        │
│  + Add service   (upsell mid-visit)    │
│                                        │
│         [ Complete Booking ]           │
└────────────────────────────────────────┘
```

On complete: apply the deposit, record the `Payment` row with the real collected amount and method, write the ledger entries, award loyalty points, and trigger the review request.

"+ Add service" matters — a customer in the chair who says "trim the beard too" is your cheapest revenue, and it needs `AppointmentItem` from [BOOKING.md](./BOOKING.md).

**Verify:** complete a booking end to end; ledger sums to zero; owner's Earnings shows the net; payout batch picks it up.
**Commit:** `feat(payment): commission rules, ledger and salon payouts`
